  ...
  const { data: contacts = [], isLoading: isLoadingContacts } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (profilesError) {
          console.warn("[useMessages] Profiles error:", profilesError.message);
          return [];
        }
        ...
        if (unreadError) {
          console.warn("[useMessages] Unread error:", unreadError.message);
        }
        ...
        console.error("[useMessages] Contacts error:", error);
        return [];
      }
    },
    enabled: !!user?.id,
  ...
  const { data: allConversations = [], isLoading: isLoadingSupervision } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (error) {
          console.warn("[useMessages] Supervision error:", error.message);
          return [];
        }
        ...
        console.error("[useMessages] Supervision error:", error);
        return [];
      }
    },
  ...
  const { data: chatMessages = [], isLoading: isLoadingMessages } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (error) {
          console.warn("[useMessages] Chat error:", error.message);
          return [];
        }
        return data || [];
      } catch (error) {
        console.error("[useMessages] Chat error:", error);
        return [];
      }
    },
    ...
    queryFn: async () => {
      try {
        const { count, error } = await supabase
          ...
          .eq('is_read', false);
        if (error) {
          console.warn("[useUnreadCount] Error:", error.message);
          return 0;
        }
        return count || 0;
      } catch (error) {
