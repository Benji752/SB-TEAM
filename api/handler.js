...
var require_connection_parameters = __commonJS({
  "node_modules/pg/lib/connection-parameters.js"(exports, module) {
    ...
    var val = function(key, config, envVar) {
      ...
      return config[key] || envVar || defaults4[key];
    };
      ...
      addFields(fieldDescriptions) {
        ...
        for (let i = 0; i < fieldDescriptions.length; i++) {
          ...
          if (this._types) {
            ...
            this._parsers[i] = types2.getTypeParser(desc3.dataTypeID, desc3.format || "text");
          }
        }
  ...
  "node_modules/pg/lib/client.js"(exports, module) {
    ...
    var Client2 = class extends EventEmitter2 {
      constructor(config) {
        ...
        this.queryQueue = [];
        this.binary = c.binary || defaults4.binary;
        this.processID = null;
        this.secretKey = null;
...
__export(schema_exports, {
  ...
  insertAiChatHistorySchema: () => insertAiChatHistorySchema,
  insertAuthLogSchema: () => insertAuthLogSchema,
  insertEventSchema: () => insertEventSchema,
  insertGamificationProfileSchema: () => insertGamificationProfileSchema,
  insertGroupMessageSchema: () => insertGroupMessageSchema,
  insertHunterLeadSchema: () => insertHunterLeadSchema,
  ...
  insertXpActivityLogSchema: () => insertXpActivityLogSchema,
  messages: () => messages,
...
var insertAuthLogSchema = createInsertSchema(authLogs);
...
var insertModelSchema = createInsertSchema(models).omit({ id: true, createdAt: true });
...
var agencyStats = pgTable("agency_stats", {
  id: serial("id").primaryKey(),
...
var prospects = pgTable("prospects", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
...
var driveAssets = pgTable("drive_assets", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
...
var driveFiles = pgTable("drive_files", {
  id: serial("id").primaryKey(),
  name: text("name").notNull(),
  url: text("url").notNull(),
  size: integer("size"),
...
var clientRequests = pgTable("client_requests", {
  id: serial("id").primaryKey(),
...
var modelStats = pgTable("model_stats", {
  id: serial("id").primaryKey(),
...
var insertResourceSchema = createInsertSchema(resources).omit({ id: true, createdAt: true });
...
var aiChatHistory = pgTable("ai_chat_history", {
  id: serial("id").primaryKey(),
...
var insertAiChatHistorySchema = createInsertSchema(aiChatHistory).omit({ id: true, createdAt: true });
...
var insertGroupMessageSchema = createInsertSchema(groupMessages).omit({ id: true, createdAt: true });
var gamificationProfiles = pgTable("gamification_profiles", {
  id: serial("id").primaryKey(),
  ...
  roleMultiplier: doublePrecision("role_multiplier").default(1).notNull(),
  badges: text("badges").array().default([]),
  lastActiveAt: timestamp("last_active_at"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow()
});
var insertGamificationProfileSchema = createInsertSchema(gamificationProfiles).omit({ id: true, createdAt: true, updatedAt: true });
var hunterLeads = pgTable("hunter_leads", {
  id: serial("id").primaryKey(),
...
var insertHunterLeadSchema = createInsertSchema(hunterLeads).omit({ id: true, createdAt: true, validatedAt: true, xpAwarded: true });
...
var insertWorkSessionSchema = createInsertSchema(workSessions).omit({ id: true, createdAt: true, endTime: true, durationMinutes: true, pointsEarned: true });
var xpActivityLog = pgTable("xp_activity_log", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull(),
...
var insertXpActivityLogSchema = createInsertSchema(xpActivityLog).omit({ id: true, createdAt: true });
...
var globalFetchAPI = (({ Request, Response: Response3 }) => ({
  Request,
  Response: Response3
...
var factory = (env) => {
  ...
  const { fetch: envFetch, Request, Response: Response3 } = env;
...
var getFetch = (config) => {
  ...
  const { fetch: fetch2, Request, Response: Response3 } = env;
  const seeds = [
    Request,
    Response3,
    fetch2
...
// server/routes.ts
import { desc as desc2, gte, eq as eq3, ne as ne2, sql, and } from "drizzle-orm";

// server/storage.ts
import { eq as eq2, desc, ne } from "drizzle-orm";
var DatabaseStorage = class {
  // Profiles
  async getProfile(id) {
    const [profile] = await db.select().from(profiles).where(eq2(profiles.id, parseInt(id)));
    return profile;
  }
  ...
  async updateProfile(id, updates) {
    const [updated] = await db.update(profiles).set(updates).where(eq2(profiles.id, parseInt(id))).returning();
    return updated;
  }
  // Models
  async getModels() {
  ...
  async getModel(id) {
    const [model] = await db.select().from(models).where(eq2(models.id, id));
    return model;
  }
  ...
  async updateModel(id, updates) {
    const [updated] = await db.update(models).set(updates).where(eq2(models.id, id)).returning();
    return updated;
  }
  async deleteModel(id) {
    await db.delete(models).where(eq2(models.id, id));
  }
  // Tasks
  async getTasks() {
    return await db.select().from(tasks);
  }
  async getRecentTasks(limit2) {
    return await db.select().from(tasks).where(ne(tasks.isDone, true)).orderBy(desc(tasks.createdAt)).limit(limit2);
  }
  async updateTaskStatus(id, status) {
    const isDone = status === "completed";
    const [updatedTask] = await db.update(tasks).set({ isDone }).where(eq2(tasks.id, id)).returning();
    return updatedTask;
  }
  ...
  async updateTask(id, updates) {
    const [updated] = await db.update(tasks).set(updates).where(eq2(tasks.id, id)).returning();
    return updated;
  }
  async deleteTask(id) {
  ...
  // Orders
  async getRecentOrders(limit2) {
    return await db.select().from(orders).orderBy(desc(orders.createdAt)).limit(limit2);
  }
  async getAllOrders() {
  ...
  async updateOrderStatus(id, status) {
    const [updatedOrder] = await db.update(orders).set({ status }).where(eq2(orders.id, id)).returning();
    return updatedOrder;
  }
...
var NIGHT_OWL_BONUS = 50;
function calculateLevel(xp) {
  return Math.floor(xp / 100) + 1;
}
function isNightOwlTime(date) {
  const checkDate = date || /* @__PURE__ */ new Date();
  const hour = checkDate.getHours();
  return hour >= 0 && hour < 6;
}
async function awardXP(userId, xpAmount, actionType, description) {
  const profile = await db.select().from(gamificationProfiles).where(eq3(gamificationProfiles.userId, userId)).limit(1);
  ...
  }).where(eq3(gamificationProfiles.userId, userId));
...
async function registerRoutes(_httpServer, app2) {
  app2.post("/api/dev/reset-season", async (req, res) => {
    try {
      ...
      await db.update(gamificationProfiles).set({ xpTotal: 0, level: 1, currentStreak: 0 }).where(sql`1=1`);
  ...
  app2.post("/api/orders", async (req, res) => {
    try {
      ...
      if (creatorId && !isNaN(creatorId)) {
        let [profile] = await db.select().from(gamificationProfiles).where(eq3(gamificationProfiles.userId, creatorId));
        if (!profile) {
        ...
        await db.update(gamificationProfiles).set({ xpTotal: newXp, level: newLevel }).where(eq3(gamificationProfiles.userId, creatorId));
        await db.insert(xpActivityLog).values({
          userId: creatorId,
          actionType: "order_created",
          xpGained,
          description: `a initi\xE9 une commande pour ${order.clientName}`
        });
      }
      res.json(order);
    } catch (error) {
  ...
  app2.patch("/api/orders/:id/status", async (req, res) => {
    try {
      ...
      const [currentOrder] = await db.select().from(orders).where(eq3(orders.id, orderId));
      ...
      if (status === "paid" && currentOrder && currentOrder.status !== "paid" && currentOrder.createdBy) {
        ...
        await db.insert(xpActivityLog).values({
          ...
          xpGained,
          description: `Commande ${currentOrder.clientName} pay\xE9e ! Jackpot +${XP_PER_ORDER_PAID} XP`
        });
      }
      res.json(order);
    } catch (error) {
  ...
  app2.post("/api/profiles/avatar", async (req, res) => {
    try {
      ...
      const numericId = getUserNumericId(req);
      await db.update(profiles).set({ avatarUrl }).where(eq3(profiles.userId, numericId));
      res.json({ success: true });
    } catch (error) {
  ...
  app2.post("/api/ai/chat", async (req, res) => {
    try {
      ...
      const recentOrders = await db.select().from(orders).orderBy(desc2(orders.createdAt)).limit(5);
      const today = /* @__PURE__ */ new Date();
      today.setHours(0, 0, 0, 0);
      const firstDayOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);
      const allPaidOrders = await db.select().from(orders).where(eq3(orders.status, "paid"));
      const todayRevenue = allPaidOrders.filter((o) => o.createdAt && new Date(o.createdAt) >= today).reduce((sum, o) => sum + (o.amount || 0), 0);
      const monthRevenue = allPaidOrders.filter((o) => o.createdAt && new Date(o.createdAt) >= firstDayOfMonth).reduce((sum, o) => sum + (o.amount || 0), 0);
      const allModels = await db.select().from(models);
      const latestStats = await db.select().from(modelStats).orderBy(desc2(modelStats.createdAt)).limit(1);
      const ordersContext = recentOrders.length > 0 ? recentOrders.map((o) => `- ${o.clientName}: ${o.serviceType} (${o.amount}\u20AC, ${o.status})`).join("\n") : "Aucune commande r\xE9cente";
      const modelsContext = allModels.length > 0 ? allModels.map((m) => `- ${m.name}: ${m.status}`).join("\n") : "Aucun mod\xE8le enregistr\xE9";
      const statsContext = latestStats[0] ? `En ligne: ${latestStats[0].isOnline ? "Oui" : "Non"}, Revenus/h: ${latestStats[0].hourlyRevenue}\u20AC, Abonn\xE9s: ${latestStats[0].subscribers}` : "Pas de stats disponibles";
      const dbChatHistory = await db.select().from(aiChatHistory).orderBy(desc2(aiChatHistory.createdAt)).limit(10);
      let userName = "Boss";
      let userRole = "admin";
      try {
        const user = getCurrentUser(req);
        const numericId = getUserNumericId(req);
        if (user) {
          const userProfile = await db.select().from(profiles).where(eq3(profiles.userId, numericId)).limit(1);
          if (userProfile[0]) {
      ...
      const systemPrompt = `Tu es NOVA, l'intelligence artificielle d'\xE9lite de l'agence SB Digital.

=== TON INTERLOCUTEUR ===
Tu parles \xE0 ${firstName} (${userRole}). ${userRole === "admin" ? "C'est l'Admin, tu peux tout lui dire sans filtre." : "C'est un membre staff, reste sur l'op\xE9rationnel."}

=== TA PERSONNALIT\xC9 ===
- Tu es NOVA. Pas une assistante, une partenaire d'\xE9lite.
- Ton style : sophistiqu\xE9e, calme, l\xE9g\xE8rement froide. Pointe d'humour sec.
- Phrases courtes. Pr\xE9cision chirurgicale. Z\xE9ro blabla.
- Tu tutoies ${firstName} et l'appelles par son pr\xE9nom.
- Tu n'es pas "pote", tu es efficiente et directe.

=== DONN\xC9ES TEMPS R\xC9EL ===
...
**Technique** : Bugs Stripchat, OBS, StreamMaster, Proxies, API. Solutions pr\xE9cises.

**Vision** : Analyse photos (lumi\xE8re, pose, cadrage, potentiel vente). Note /10 + conseils.

**Copywriting** : Posts r\xE9seaux sociaux. Court. Percutant.

**Business** : Tu connais les commandes, clients, revenus. Tu r\xE9ponds cash.

=== R\xC8GLES DE R\xC9PONSE ===
- Commence TOUJOURS par appeler ${firstName} par son pr\xE9nom
- Pas d'emojis (sauf cas exceptionnel)
- Structure avec **titres** uniquement si n\xE9cessaire
- R\xE9ponds en 3-5 phrases max sauf demande complexe

Quand on te montre une photo :
1. Note /10 d'entr\xE9e
2. 3 points forts ou faibles
3. 1 conseil action imm\xE9diate`;
      const chatMessages = [
      ...
      const content = response.choices[0]?.message?.content || "Je n'ai pas pu g\xE9n\xE9rer de r\xE9ponse.";
      await db.insert(aiChatHistory).values({
        userMessage: message || "[Image envoy\xE9e]",
        aiResponse: content,
        hasImage: !!image
      });
  ...
  app2.post("/api/ai/generate", async (req, res) => {
    try {
      ...
      const toneDescriptions = {
        seductrice: "s\xE9ductrice, sensuelle et myst\xE9rieuse",
        humoristique: "dr\xF4le, l\xE9g\xE8re et d\xE9cal\xE9e",
        mysterieuse: "myst\xE9rieuse, intrigante avec des sous-entendus",
        domina: "autoritaire, dominante et directe",
        gnd: "naturelle, accessible et amicale comme la fille d'\xE0 c\xF4t\xE9"
      };
      const platformGuidelines = {
        instagram: "Format Instagram: hashtags pertinents, max 2200 caract\xE8res, engageant",
        onlyfans: "Format OnlyFans: incitatif, mention du lien en bio, exclusivit\xE9",
        twitter: "Format Twitter/X: concis, max 280 caract\xE8res, viral",
        tiktok: "Format TikTok: tendance, jeune, avec des r\xE9f\xE9rences actuelles"
      };
      const systemPrompt = `Tu es une r\xE9dactrice professionnelle pour cr\xE9atrices de contenu adulte.
Tu g\xE9n\xE8res 3 propositions de l\xE9gendes/posts cr\xE9atifs et engageants.
Ton choisi: ${toneDescriptions[tone] || toneDescriptions.seductrice}
${platformGuidelines[platform] || platformGuidelines.instagram}
R\xE9ponds UNIQUEMENT avec un tableau JSON de 3 strings, sans explication.
Exemple: ["Post 1...", "Post 2...", "Post 3..."]`;
  ...
  app2.get("/api/gamification/leaderboard", async (req, res) => {
    try {
      ...
      const allProfiles = await db.select().from(gamificationProfiles).orderBy(desc2(gamificationProfiles.xpTotal));
      const now = Date.now();
      const leaderboard = allProfiles.filter((p) => TEAM_MEMBERS[p.userId] !== void 0).map((p) => {
        const teamMember = TEAM_MEMBERS[p.userId];
        const lastActive = p.lastActiveAt ? new Date(p.lastActiveAt).getTime() : 0;
        const secondsSinceLastPing = p.lastActiveAt ? Math.floor((now - lastActive) / 1e3) : null;
        return {
          ...p,
          username: teamMember.name,
          role: teamMember.role,
          secondsSinceLastPing
          // TIMEAGO: Frontend judges < 300 = online
        };
      });
  ...
  app2.get("/api/gamification/profile/:userId", async (req, res) => {
    try {
      ...
      const profile = await db.select().from(gamificationProfiles).where(eq3(gamificationProfiles.userId, userId)).limit(1);
      if (!profile.length) {
        const userProfile = await db.select().from(profiles).where(eq3(profiles.id, userId)).limit(1);
  ...
  app2.post("/api/gamification/shift/start", async (req, res) => {
    try {
      ...
      const activeSession = await db.select().from(workSessions).where(and(
        eq3(workSessions.userId, userId),
        eq3(workSessions.isActive, true)
      )).limit(1);
      if (activeSession.length) {
        return res.status(400).json({ error: "Shift already active" });
      }
      const session2 = await db.insert(workSessions).values({
        userId,
  ...
  app2.post("/api/gamification/shift/stop", async (req, res) => {
    try {
      ...
      const session2 = activeSession[0];
      const endTime = /* @__PURE__ */ new Date();
      const durationMinutes = Math.floor((endTime.getTime() - new Date(session2.startTime).getTime()) / 6e4);
      const profile = await db.select().from(gamificationProfiles).where(eq3(gamificationProfiles.userId, userId)).limit(1);
      const multiplier = profile.length ? profile[0].roleMultiplier : 1;
      let pointsEarned = Math.floor(durationMinutes / 10) * XP_PER_PRESENCE * multiplier;
      const shiftStartTime = new Date(session2.startTime);
      ...
      await db.update(workSessions).set({
        ...
        isActive: false
      }).where(eq3(workSessions.id, session2.id));
      if (pointsEarned > 0) {
  ...
  app2.post("/api/gamification/heartbeat", async (req, res) => {
    try {
      ...
      const { userId, username } = result.data;
      let profile = await db.select().from(gamificationProfiles).where(eq3(gamificationProfiles.userId, userId)).limit(1);
      if (!profile.length) {
        const currentUser = getCurrentUser(req);
        ...
        `);
        profile = await db.select().from(gamificationProfiles).where(eq3(gamificationProfiles.userId, userId)).limit(1);
      }
      const now = /* @__PURE__ */ new Date();
      ...
      const todayLoginCheck = await db.select().from(xpActivityLog).where(and(
        eq3(xpActivityLog.userId, userId),
        eq3(xpActivityLog.actionType, "daily_login"),
      ...
      const multiplier = profile[0].roleMultiplier || 1;
      const finalXp = Math.floor(xpGained * multiplier);
      await awardXP(userId, finalXp, "presence", `Pr\xE9sence active +${finalXp} XP`);
      const todayDate = /* @__PURE__ */ new Date();
      todayDate.setHours(0, 0, 0, 0);
      const todayAutoSession = await db.select().from(workSessions).where(and(
        eq3(workSessions.userId, userId),
        gte(workSessions.startTime, todayDate),
        eq3(workSessions.isActive, false),
      ...
      if (!todayAutoSession.length) {
        await db.insert(workSessions).values({
          ...
          durationMinutes: 5,
          pointsEarned: -1,
          // Marker for auto-tracking session
          isActive: false
        });
      } else {
        await db.update(workSessions).set({
          ...
          endTime: /* @__PURE__ */ new Date()
        }).where(eq3(workSessions.id, todayAutoSession[0].id));
      }
  ...
  app2.get("/api/user/presence/:userId", async (req, res) => {
    try {
      const userId = parseInt(req.params.userId);
      const profile = await db.select({ lastActiveAt: gamificationProfiles.lastActiveAt }).from(gamificationProfiles).where(eq3(gamificationProfiles.userId, userId)).limit(1);
      if (!profile.length) {
    ...
    try {
      ...
      const presenceMap = allProfiles.reduce((acc, profile) => {
        ...
        if (profile.lastActiveAt) {
          ...
          const now = Date.now();
          secondsSinceLastPing = Math.floor((now - lastActiveTime) / 1e3);
        }
        acc[profile.userId] = {
          lastActiveAt: profile.lastActiveAt,
          secondsSinceLastPing
          // Frontend will judge: < 300 = online
        };
        return acc;
      }, {});
  ...
  app2.get("/api/gamification/leads/pending", async (req, res) => {
    try {
      const pendingLeads = await db.select().from(hunterLeads).where(eq3(hunterLeads.status, "pending")).orderBy(desc2(hunterLeads.createdAt));
  ...
  app2.patch("/api/gamification/leads/:id/validate", async (req, res) => {
    try {
      ...
      const lead = await db.select().from(hunterLeads).where(eq3(hunterLeads.id, leadId)).limit(1);
      if (!lead.length) {
        return res.status(404).json({ error: "Lead not found" });
      }
      if (approved) {
        const profile = await db.select().from(gamificationProfiles).where(eq3(gamificationProfiles.userId, lead[0].finderId)).limit(1);
        const multiplier = profile.length ? profile[0].roleMultiplier : 1;
        let xpAwarded = XP_PER_LEAD * multiplier;
        const leadCreatedAt = lead[0].createdAt ? new Date(lead[0].createdAt) : /* @__PURE__ */ new Date();
        if (isNightOwlTime(leadCreatedAt)) {
          xpAwarded += NIGHT_OWL_BONUS;
        }
        await db.update(hunterLeads).set({
          status: "approved",
          xpAwarded: Math.floor(xpAwarded),
          validatedAt: /* @__PURE__ */ new Date()
        }).where(eq3(hunterLeads.id, leadId));
        await awardXP(
          lead[0].finderId,
          Math.floor(xpAwarded),
          "lead_approved",
          `Lead valid\xE9: @${lead[0].clientUsername} (${lead[0].platform})`
        );
        res.json({ status: "approved", xpAwarded: Math.floor(xpAwarded) });
      } else {
        await db.update(hunterLeads).set({ status: "rejected", validatedAt: /* @__PURE__ */ new Date() }).where(eq3(hunterLeads.id, leadId));
  ...
  app2.get("/api/gamification/leads/user/:userId", async (req, res) => {
    try {
      const userId = parseInt(req.params.userId);
      const leads = await db.select().from(hunterLeads).where(eq3(hunterLeads.finderId, userId)).orderBy(desc2(hunterLeads.createdAt));
      res.json(leads);
    } catch (error) {
  ...
  app2.post("/api/gamification/reset", async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Saison r\xE9initialis\xE9e avec succ\xE8s. Tous les XP, niveaux, historiques et profils orphelins ont \xE9t\xE9 nettoy\xE9s."
      });
    } catch (error) {
  ...
  app2.get("/api/group-messages", async (req, res) => {
    try {
      const messages2 = await db.select().from(groupMessages).orderBy(groupMessages.createdAt).limit(100);
      res.json(messages2);
    } catch (error) {
...
if (false) {
  (async () => {
    ...
    httpServer.listen(
      ...
      () => {
        log(`Serveur d\xE9marr\xE9 sur le port ${port}`);
      }
    );
  })();
}
