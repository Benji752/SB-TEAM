...
// server/routes.ts
import { desc as desc2, gte, eq as eq2, ne as ne2, sql, and } from "drizzle-orm";
...
var DatabaseStorage = class {
  ...
  async updateProfile(id, updates) {
    const [updated] = await db.update(profiles).set(updates).where(eq(profiles.id, parseInt(id))).returning();
    return updated;
  }
  // Models
  async getModels() {
  ...
  async updateTaskStatus(id, status) {
    ...
    const [updatedTask] = await db.update(tasks).set({ isDone }).where(eq(tasks.id, id)).returning();
    return updatedTask;
  }
...
async function awardXP(userId, xpAmount, actionType, description) {
  const profile = await db.select().from(gamificationProfiles).where(eq2(gamificationProfiles.userId, userId)).limit(1);
  ...
  }).where(eq2(gamificationProfiles.userId, userId));
...
async function registerRoutes(_httpServer, app2) {
  ...
  app2.post("/api/orders", async (req, res) => {
    try {
      ...
      if (creatorId && !isNaN(creatorId)) {
        let [profile] = await db.select().from(gamificationProfiles).where(eq2(gamificationProfiles.userId, creatorId));
        if (!profile) {
        ...
        await db.update(gamificationProfiles).set({ xpTotal: newXp, level: newLevel }).where(eq2(gamificationProfiles.userId, creatorId));
        await db.insert(xpActivityLog).values({
          userId: creatorId,
  ...
  app2.patch("/api/orders/:id/status", async (req, res) => {
    try {
      ...
      const [currentOrder] = await db.select().from(orders).where(eq2(orders.id, orderId));
  ...
  app2.post("/api/ai/chat", async (req, res) => {
    try {
      ...
      const allPaidOrders = await db.select().from(orders).where(eq2(orders.status, "paid"));
...
Exemple: ["Post 1...", "Post 2...", "Post 3..."]`;
  ...
  app2.post("/api/gamification/shift/start", async (req, res) => {
    try {
      ...
      const activeSession = await db.select().from(workSessions).where(and(
        eq2(workSessions.userId, userId),
        eq2(workSessions.isActive, true)
      )).limit(1);
  ...
  app2.post("/api/gamification/shift/stop", async (req, res) => {
    try {
      ...
      await db.update(workSessions).set({
        ...
        isActive: false
      }).where(eq2(workSessions.id, session2.id));
      if (pointsEarned > 0) {
  ...
  app2.post("/api/gamification/heartbeat", async (req, res) => {
    try {
      ...
      const todayLoginCheck = await db.select().from(xpActivityLog).where(and(
        eq2(xpActivityLog.userId, userId),
        eq2(xpActivityLog.actionType, "daily_login"),
      ...
      const todayAutoSession = await db.select().from(workSessions).where(and(
        ...
        eq2(workSessions.isActive, false),
      ...
      if (!todayAutoSession.length) {
        ...
        await db.update(workSessions).set({
          ...
          endTime: /* @__PURE__ */ new Date()
        }).where(eq2(workSessions.id, todayAutoSession[0].id));
      }
  ...
  app2.get("/api/user/presence/:userId", async (req, res) => {
    try {
      const userId = parseInt(req.params.userId);
      const profile = await db.select({ lastActiveAt: gamificationProfiles.lastActiveAt }).from(gamificationProfiles).where(eq2(gamificationProfiles.userId, userId)).limit(1);
      if (!profile.length) {
  ...
  app2.patch("/api/gamification/leads/:id/validate", async (req, res) => {
    try {
      ...
      const lead = await db.select().from(hunterLeads).where(eq2(hunterLeads.id, leadId)).limit(1);
      if (!lead.length) {
        return res.status(404).json({ error: "Lead not found" });
      }
      if (approved) {
        const profile = await db.select().from(gamificationProfiles).where(eq2(gamificationProfiles.userId, lead[0].finderId)).limit(1);
        const multiplier = profile.length ? profile[0].roleMultiplier : 1;
        ...
        }).where(eq2(hunterLeads.id, leadId));
        await awardXP(
          lead[0].finderId,
        ...
        await db.update(hunterLeads).set({ status: "rejected", validatedAt: /* @__PURE__ */ new Date() }).where(eq2(hunterLeads.id, leadId));
